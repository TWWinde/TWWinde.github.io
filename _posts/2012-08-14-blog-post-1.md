...
### 4.7 Marginalization Trick

The **marginalization trick** allows us to train the model without computing the marginal vector field explicitly.

Instead we:

1. Sample $$z \sim p_{\text{data}}$$
2. Sample $$X_t \sim p_t(X_t | z)$$
3. Compute the conditional target

$$u_t^{\text{target}}(X_t | z)$$

4. Train a neural network $$u_t^\theta(X_t)$$ to predict it.

Because of the regression property of MSE:

$$
\arg\min_u \mathbb{E}\left[ \|u - Y\|^2 \right] = \mathbb{E}[Y]
$$

the optimal predictor satisfies

$$
u_t^\theta(X_t) = \mathbb{E} \left[ u_t^{\text{target}}(X_t | z) \mid X_t \right]
$$

Thus the network learns the **marginal vector field**.
